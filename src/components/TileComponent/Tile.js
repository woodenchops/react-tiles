...
const Tile = ({item, index, handleDropDown, handleDropDownKeyPress}) => {
    return ( 
        ...
        <div className={"tile-section " + (item.isActive ? "activeTile" : "")} tabIndex="0" aria-label={(item.isActive) ? (`${item.title} is open`) : (`${item.title} is closed`) } aria-live="assertive" onKeyPress={(e) => {handleDropDownKeyPress(index, e)}} onClick={()=>{handleDropDown(index)}}>
